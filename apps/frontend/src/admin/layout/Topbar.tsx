...
export function Topbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-(--color-silver-light) bg-white px-4 md:px-6">
      ...
      <NotificationBell apiClient={apiClient} basePath="/notifications/admin" queryKey="admin-notifications" />
      <div className="relative">
        ...
        >
          ...
          <div className="hidden text-right sm:block">
            <div className="text-sm font-medium text-(--color-navy)">{user?.name}</div>
